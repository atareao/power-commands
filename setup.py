...
def update_desktop_file_fp():
    ...
    for filedesktopin in glob.glob('*.desktop.in'):
        print('==============================')
        print('Doing: {0}'.format(filedesktopin))
        desktopfile = ConfigParser.ConfigParser()
        desktopfile.optionxform = str
        desktopfile.readfp(codecs.open(filedesktopin,
                                       encoding='utf-8',
                                       mode='r'))
        if len(lns) > 0:
            for entry in desktopfile.items('Desktop Entry'):
                if entry[0].startswith('_'):
                    for ln in lns:
                        desktopfile.set('Desktop Entry', '$%s[%s]' % (
                            entry[0][1:], ln), "_('%s')" % entry[1])
    ...
    def run(self):
        for lang in os.listdir('build/locale-langpack/'):
            lang_dir = os.path.join(
                '/opt/extras.ubuntu.com/power-commands/share',
                'locale-langpack',
                lang,
                'LC_MESSAGES')
            lang_file = os.path.join('build',
                                     'locale-langpack',
                                     lang,
                                     'LC_MESSAGES',
